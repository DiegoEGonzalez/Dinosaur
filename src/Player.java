...
public abstract class Player {
    ...
    public Player(Grid world, String location, int x, int y) {
        ...
        try {
            ...
            w = img.getWidth() / 10;
            for (int a = 0; a < 8; a++) {
                walk.add(ImageIO.read(new File("src/DinoR" + (1 + a)+".png")));
            }
            rj = ImageIO.read(new File("src/DinoRJ.png"));
    ...
    public boolean right() {
        ...
        return right;
    } //returns truse if nothing to the right and there's an available space
    ...
    public void draw(Graphics g) {
        if(jump&&alive){
           g.drawImage(rj, (x - 20) * 10, (y - 20) * 10, null);
           frame = 0;
        } else if (xDirection != 0&&alive&&!down()) {
            g.drawImage(walk.get(frame), (x - 20) * 10, (y - 20) * 10, null);
            frame++;
            if (frame == 8)
                frame = 0;
        } else {
